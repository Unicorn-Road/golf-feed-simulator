...
**Parameters:**
- `progress` (optional): Number between 0-100 representing tournament completion percentage. Defaults to 0.
...
```bash
# Get initial state (tee times only)
curl http://localhost:3000/api/feed?progress=0

# Get mid-tournament state (Round 2 in progress)
curl http://localhost:3000/api/feed?progress=37.5
